...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    *pot, Z, observed, expected;
    ...
    /* Compute Sizes */
    nInstances = mxGetDimensions(prhs[6])[0];
        ...
        for(n = 0; n < nNodes; n++)
        {
            ...
            for(edgeInd = V[n]; edgeInd < V[n+1]; edgeInd++)
            {
                ...
                if(n == edgeEnds[e])
                {
                    for(s = 0; s < nStates[n]; s++)
                    {
                        pot[s] *= edgePot[s + maxState*(y[i + nInstances*n2] + maxState*e)];
                    }
                }
                else
                {
                    for(s = 0; s < nStates[n]; s++)
                    {
                        pot[s] *= edgePot[y[i + nInstances*n1] + maxState*(s + maxState*e)];
                    }
                }
            }
                ...
                else /* (~ising) */
                {
                    for(s = 0; s < nStates[n]; s++)
                    {
                        ...
                        for(p = 0; p < nEdgeFeatures; p++)
                        {
                            if (tieEdges)
                            {
                                gv[p + nEdgeFeatures*sInd] -= (observed-expected)*Xedge[p + nEdgeFeatures*e];
                            }
                            else
                            {
                                gv[p + nEdgeFeatures*(sInd + (maxState*maxState-1)*e)] -= (observed-expected)*Xedge[p + nEdgeFeatures*e];
                            }
                        }
                    }
                }

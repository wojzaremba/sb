...
for d = 1:length(ds)
    ds{d}
    [X,y,k,w_init,funObj] = loadSoftmax(ds{d},zeroClass);
    for o = 1:length(options)
        if TR(o) == -1
            [w fvals evals(1,o)] = minimize(w_init, func2str(funObj), -250,X,y,k);
            fval(1,o) = fvals(end);
        elseif TR(o) == 0
            [w fval(1,o) exitflag output] = minFunc(funObj,w_init,options{o},X,y,k);
            evals(1,o) = output.funcCount;
        else
            [w fval(1,o) exitflag output] = minFuncTR(funObj,w_init,options{o},X,y,k);
            evals(1,o) = output.funcCount;
        end
    end
    fval
...
function [X,y_mc,k,w_init,funObj] = loadSoftmax(dataFile,zeroClass)
data = loadd(dataFile);
[n,d] = size(data);
X = full(data(:,1:d-1));
y = full(data(:,d));

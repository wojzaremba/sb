...
options = {struct('classifier', @kci_classifier, 'kernel', @linear_kernel, 'range', range, 'color', 'g' ), ...
           ...
           struct('classifier', @mi_classifier, 'kernel', @empty, 'range', range, 'color', 'y' )};
       %, ... struct('classifier', @sb_classifier, 'kernel', @empty, 'range',  short_range, 'color', 'k')};
rho = {};
name = {};
w_acc = zeros(length(options), 1);

%TPR = zeros(length(classifiers),length(range));
%FPR = zeros(length(classifiers),length(range));

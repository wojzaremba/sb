...
\subsection{Kernelized partial correlation}\label{sec:kci}

Kernelized conditional independence (KCI) \cite{zhang2012kernel} is a kernelized version of aforementioned statistic. 
For characteristic kernels (e.g. like Gaussian, or Laplace), the test is consistent. Table\ref{tab:compar} compares statistical properties of available tests of independence.  Furthermore, experimental results on synthetic data indicate that KCI has the best performance in practice, as shown in the following section. %for real data distributions.

First we will show how unconditional partial correlation (i.e. regular correlation) can be kernelized, and then how partial correlation can be kernelized.  Suppose you have $n$ observations each of random variables $X$, $Y$, and $Z$, where e.g. the $i^{th}$ observation of $X$ is denoted by $X_i$.  The empirical estimate of Pearson correlation between $X$ and $Y$ can be written as $$\dfrac{\X^T\Y}{\sqrt{\X^T\X \cdot \Y^T\Y}}$$

...
for fig = 1:length(xlims)
    ...
    xlabel('FPR');
    ylabel('TPR');
    title(sprintf('ROC on CPDs generated from linear asia network, arity=%d, N=%d, eta=%.3f, alpha=%.3f, AUC=%.3f.',arity,num_samples,o.params.eta(eta_idx),o.params.alpha(alpha_idx), AUC),'fontsize',12);
    xlim(xlims{fig});
    ylim(ylims{fig});
end
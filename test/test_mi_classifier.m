...
assert(mi_classifier(emp_indep, [1, 2], opt_min) < 1e-4);
assert(mi_classifier(emp_indep, [1 2 3], opt_min) < 1e-4);

assert(mi_classifier(emp_indep, [1, 2], opt_mean) < 1e-4);
assert(mi_classifier(emp_indep, [1 2 3], opt_mean) < 1e-4);

emp_dep = [randi(3, 1, samples_size); randi(2, 1, samples_size)];
emp_dep(2, emp_dep(1, :) == 3) = 1;
emp_dep(1, emp_dep(1, :) == 3) = 2;

assert(mi_classifier(emp_dep, [1, 2], opt_min) > 0.02);
assert(mi_classifier(emp_dep, [1, 2], opt_mean) > 0.02);
...
emp_dep(emp_dep == -1) = 2;
assert(mi_classifier(emp_dep, [1, 2], opt_min) > 0.02);
assert(mi_classifier(emp_dep, [1 2 3], opt_min) < 1e-4);
assert(mi_classifier(emp_dep, [1, 2], opt_mean) > 0.02);
assert(mi_classifier(emp_dep, [1 2 3], opt_mean) < 1e-4);

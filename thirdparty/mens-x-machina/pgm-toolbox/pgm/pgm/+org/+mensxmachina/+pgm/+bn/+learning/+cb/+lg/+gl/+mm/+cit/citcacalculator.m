classdef(Sealed) citcacalculator < org.mensxmachina.pgm.bn.learning.cb.lg.gl.mm.cacalculator
...
%   ALPHA is a numeric real scalar in range [0, 1].
    ...
    % parse input
    assert(isa(CITRCApplier, 'org.mensxmachina.stats.tests.ci.citrcapplier') && isscalar(CITRCApplier));
    assert(isa(CITPValueEstimator, 'org.mensxmachina.stats.tests.ci.citpvalueestimator') && isscalar(CITPValueEstimator) && CITPValueEstimator.nVars == CITRCApplier.nVars);

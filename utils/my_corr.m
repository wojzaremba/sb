function [coef, pval] = my_corr(x, varargin)
...
%   $Revision: 1.1.6.8 $  $Date: 2009/05/07 18:30:42 $
...
if nargout > 1
    ...
    if corrXX
         % Do the lower triangle and diagonal only, we'll reflect into
         % the upper.  Need to do the diagonal explicitly because those
...
for i = 1:p1
    % loop from 1:p2 for corr(x,y), from 1:i for corr(x)
    j1 = p2*(1-corrXX) + i*corrXX;
    for j = 1:j1
        xi = x(:,i);
        yj = y(:,j);
    ...
    % Force a one on the diagonal (except for Kendall when there's ties),
    % but preserve NaNs.  This relies on having most recently computed the
    % diagonal element.
    if corrXX && ~isnan(coef(i,i)) && ~(type == 'k' && ties)
        coef(i,i) = 1;
    end
end
...
% If this is an autocorrelation, reflect the lower correlations and
% p-values into the upper triangle.  Leave the diagonal alone.
if corrXX
    coef = tril(coef) + tril(coef,-1)';
    if nargout > 1
        pval = tril(pval) + tril(pval,-1)';
    end
end
